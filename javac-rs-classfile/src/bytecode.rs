...
use crate::attribute::{NamedAttribute, AttributeCreateError, Attributable, AttributeAddError, ExceptionTableInfo, TryIntoNamedAttribute};
use crate::{ConstPool, JvmVecU4, JvmVecU2, JvmVecStoreError, ConstPoolIndex, ConstClassInfo, ConstFieldRefInfo, ConstInvokeDynamicInfo, ConstInterfaceMethodRefInfo, ConstPoolEntryInfo, TypeDescriptor, ConstMethodRefInfo};
use std::cmp::max;
...
impl ExceptionTable {
    pub fn into_attribute_analog(self, const_pool: &mut ConstPool) -> Result<ExceptionTableInfo, AttributeCreateError> {
        ExceptionTableInfo::new(
            const_pool, self.start_pc, self.end_pc, self.handler_prc, self.catch_type,
        )
    }
}
...
impl Bytecode {
    ...
    fn check_local_index(&self, local_index: u16) -> Result<(), BytecodeUpdateError> {
        if local_index >= self.max_locals { Err(BytecodeUpdateError::LocalIndexOutOfBounds) } else { Ok(()) }
    }

    fn stack_update(&mut self, dec: u16, inc: u16) -> Result<(), BytecodeUpdateError> {
        match (dec, inc) {
            (0, 0) => {} // no need to verify stack as no changes happen
            (dec, 0) => self.stack = self.stack.checked_sub(dec)
                .ok_or(BytecodeUpdateError::CorruptedStack)?,
            (0, inc) => {
                self.stack = self.stack.checked_add(inc)
                    .ok_or(BytecodeUpdateError::CorruptedStack)?;
                self.max_stack = max(self.max_stack, self.stack);
            }
            (inc, dec) => {
                self.stack = self.stack.checked_sub(dec)
    ...
    #[inline(always)] // used only privately to reduce code duplication
    fn instr_load(&mut self, generic_opcode: u8, specific_0_opcode: u8, index: u16, fat: bool)
                  -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    #[inline(always)] // used only privately to reduce code duplication
    fn instr_store(&mut self, generic_opcode: u8, specific_0_opcode: u8, index: u16, fat: bool)
                   -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.check_local_index(index)
...
impl Bytecode {
    ...
    pub fn instr_anewarray(&mut self, component_type: ConstPoolIndex<ConstClassInfo>) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(0, 1)
    ...
    pub fn instr_dup_x2(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        ...
        self.stack_update(3, 4)
            .and(self.push_instr(0x5b))
    }

    pub fn instr_dup2(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 4)
            .and(self.push_instr(0x5c))
    }
    ...
    pub fn instr_dup2_x1(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        ...
        // the latter just has longs/doubles on both sides making it equal to the former
        self.stack_update(3, 5)
            .and(self.push_instr(0x5d))
    }
    ...
    pub fn instr_freturn(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
            .and(self.push_instr(0xae))
    }

    pub fn instr_fsub(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 1)
            .and(self.push_instr(0x66))
    }

    pub fn instr_getfield(&mut self, field: ConstPoolIndex<ConstFieldRefInfo>, fat: bool) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, Self::slot_size(fat))
            .and(self.push_instr(0xb4))
            .and(self.push_ops(&field.as_bytes()))
    }

    pub fn instr_getstatic(&mut self, field: ConstPoolIndex<ConstFieldRefInfo>, fat: bool) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(0, Self::slot_size(fat))
            .and(self.push_instr(0xb2))
            .and(self.push_ops(&field.as_bytes()))
    }

    pub fn instr_goto(&mut self, offset: BytecodeOffset) -> Result<BytecodeOffset, BytecodeUpdateError> {
        if offset <= u16::MAX as u32 { // goto
            self.stack_update(1, 1)
    ...
    pub fn instr_if_acmpeq(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_acmpne(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmpeq(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmpne(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmplt(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmpge(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmpgt(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_if_icmple(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 0)
    ...
    pub fn instr_ifeq(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifne(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_iflt(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifge(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifgt(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifle(&mut self, branch: u16 /* not sure why wide is not used here */)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifnonnull(&mut self, branch: u16 /* not sure why wide is not used here */)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_ifnull(&mut self, branch: u16 /* not sure why wide is not used here */)
                        -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 0)
    ...
    pub fn instr_iinc(&mut self, local_variable_index: u16, delta: i16)
                      -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_imul(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 1)
            .and(self.push_instr(0x68))
    }

    pub fn instr_ineg(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 1)
            .and(self.push_instr(0x74))
    }

    pub fn instr_instanceof(&mut self, checked_type: ConstPoolIndex<ConstClassInfo>)
                            -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(1, 1)
    ...
    pub fn instr_invokedynamic(&mut self, bootstrap_method: ConstPoolIndex<ConstInvokeDynamicInfo>, arguments_count: u8)
                               -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_invokeinterface(&mut self, method: ConstPoolIndex<ConstInterfaceMethodRefInfo>, arguments_count: u8)
                                 -> Result<BytecodeOffset, BytecodeUpdateError> {
        let arguments_count = arguments_count.checked_add(1)
    ...
    pub fn instr_invokespecial(&mut self, method: ConstPoolIndex<ConstInterfaceMethodRefInfo>, arguments_count: u8)
                               -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(arguments_count.checked_add(1)
                              .ok_or(BytecodeUpdateError::TooMuchMethodParameters)? as u16, 0)
            .and(self.push_instr(0xb7))
            .and(self.push_ops(&method.to_be_bytes()))
    }

    pub fn instr_invokestatic(&mut self, method: ConstPoolIndex<ConstInterfaceMethodRefInfo>, arguments_count: u8)
                              -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(arguments_count as u16, 0)
    ...
    pub fn instr_invokevirtual(&mut self, method: ConstPoolIndex<ConstMethodRefInfo>, arguments_count: u8)
                               -> Result<BytecodeOffset, BytecodeUpdateError> {
        println!("1) self = {:?}", self);
        let arguments_count = arguments_count.checked_add(1)
            .ok_or(BytecodeUpdateError::TooMuchMethodParameters)?;
    ...
    pub fn instr_isub(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 1)
            .and(self.push_instr(0x64))
    }

    pub fn instr_iushr(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 1)
            .and(self.push_instr(0x7c))
    }

    pub fn instr_ixor(&mut self) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(2, 1)
            .and(self.push_instr(0x82))
    }

    pub fn instr_jsr(&mut self, offset: BytecodeOffset) -> Result<BytecodeOffset, BytecodeUpdateError> {
        if offset <= u16::MAX as u32 { // jsr
            self.stack_update(1, 1)
    ...
    pub fn instr_ldc2_w<E: ConstPoolEntryInfo>(&mut self, value: ConstPoolIndex<E>)
                                               -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(0, 2)
    ...
    pub fn instr_multinewarray(&mut self, component_type: ConstPoolIndex<ConstClassInfo>, dimensions: NonZeroU8) -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_new(&mut self, component_type: ConstPoolIndex<ConstClassInfo>) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.stack_update(0, 1)
            .and(self.push_instr(0xbb))
            .and(self.push_ops(&component_type.as_bytes()))
    }

    pub fn instr_newarray(&mut self, component_type: TypeDescriptor) -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_putfield(&mut self, field: ConstPoolIndex<ConstFieldRefInfo>, fat: bool)
                          -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_putstatic(&mut self, field: ConstPoolIndex<ConstFieldRefInfo>, fat: bool)
                           -> Result<BytecodeOffset, BytecodeUpdateError> {
    ...
    pub fn instr_ret(&mut self, index: u8) -> Result<BytecodeOffset, BytecodeUpdateError> {
        self.push_instr(0xa9)
            .and(self.push_ops(&[index]))
    }
...
impl TryIntoNamedAttribute for Bytecode {
    fn try_into_named_attribute(self, const_pool: &mut ConstPool) -> Result<NamedAttribute, AttributeCreateError> {
        NamedAttribute::new_code_attribute(
            const_pool, self.max_stack, self.max_locals, self.code, self.exception_tables, self.attributes,
        )
    }
}
